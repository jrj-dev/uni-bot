#!/usr/bin/env python3
"""Generic UniFi local API client using X-API-Key authentication."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Console base URL. Defaults to UNIFI_BASE_URL.",
    )
    parser.add_argument(
        "--api-key",
        default=os.environ.get("UNIFI_API_KEY"),
        help="API key. Defaults to UNIFI_API_KEY.",
    )
    parser.add_argument(
        "--query",
        action="append",
    ...
    parser.add_argument(
        "--allow-write",
        action="store_true",
        help="Permit non-read-only methods such as POST or PATCH.",
    )
...
def parse_body(json_body: str | None) -> bytes | None:
    if json_body is None:
        return None
    try:
        parsed: Any = json.loads(json_body)
...
def main() -> int:
    ...
    base_url = require(args.base_url, "UNIFI_BASE_URL")
    api_key = require(args.api_key, "UNIFI_API_KEY")
    query_items = parse_query(args.query)
    body = parse_body(args.json_body)
